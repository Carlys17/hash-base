// Keccak-256 WebGPU Compute Shader for $HASH mining
// Runs massively parallel on GPU — each thread searches independent nonce range
...
fn keccak_f1600(s: ptr<function, array<vec2<u32>, 25>>) {
  for (var r: u32 = 0u; r < 24u; r = r + 1u) {
    ...
    let b00 = xor64((*s)[ 0], D0); let b10 = rotl64(xor64((*s)[ 1], D1),  1u);
    let b20 = rotl64(xor64((*s)[ 2], D2), 62u); let b05 = rotl64(xor64((*s)[ 3], D3), 28u);
    let b15 = rotl64(xor64((*s)[ 4], D4), 27u); let b16 = rotl64(xor64((*s)[ 5], D0), 36u);
    let b01 = rotl64(xor64((*s)[ 6], D1), 44u); let b11 = rotl64(xor64((*s)[ 7], D2),  6u);
    let b21 = rotl64(xor64((*s)[ 8], D3), 55u); let b06 = rotl64(xor64((*s)[ 9], D4), 20u);
    let b07 = rotl64(xor64((*s)[10], D0),  3u); let b17 = rotl64(xor64((*s)[11], D1), 10u);
    let b02 = rotl64(xor64((*s)[12], D2), 43u); let b12 = rotl64(xor64((*s)[13], D3), 25u);
    let b22 = rotl64(xor64((*s)[14], D4), 39u); let b23 = rotl64(xor64((*s)[15], D0), 41u);
    let b08 = rotl64(xor64((*s)[16], D1), 45u); let b18 = rotl64(xor64((*s)[17], D2), 15u);
    let b03 = rotl64(xor64((*s)[18], D3), 21u); let b13 = rotl64(xor64((*s)[19], D4),  8u);
    let b14 = rotl64(xor64((*s)[20], D0), 18u); let b24 = rotl64(xor64((*s)[21], D1),  2u);
    let b09 = rotl64(xor64((*s)[22], D2), 61u); let b19 = rotl64(xor64((*s)[23], D3), 56u);
    let b04 = rotl64(xor64((*s)[24], D4), 14u);

    (*s)[ 0] = xor64(b00, andnot64(b01, b02)); (*s)[ 1] = xor64(b01, andnot64(b02, b03));
    (*s)[ 2] = xor64(b02, andnot64(b03, b04)); (*s)[ 3] = xor64(b03, andnot64(b04, b00));
    (*s)[ 4] = xor64(b04, andnot64(b00, b01)); (*s)[ 5] = xor64(b05, andnot64(b06, b07));
    (*s)[ 6] = xor64(b06, andnot64(b07, b08)); (*s)[ 7] = xor64(b07, andnot64(b08, b09));
    (*s)[ 8] = xor64(b08, andnot64(b09, b05)); (*s)[ 9] = xor64(b09, andnot64(b05, b06));
    (*s)[10] = xor64(b10, andnot64(b11, b12)); (*s)[11] = xor64(b11, andnot64(b12, b13));
    (*s)[12] = xor64(b12, andnot64(b13, b14)); (*s)[13] = xor64(b13, andnot64(b14, b10));
    (*s)[14] = xor64(b14, andnot64(b10, b11)); (*s)[15] = xor64(b15, andnot64(b16, b17));
    (*s)[16] = xor64(b16, andnot64(b17, b18)); (*s)[17] = xor64(b17, andnot64(b18, b19));
    (*s)[18] = xor64(b18, andnot64(b19, b15)); (*s)[19] = xor64(b19, andnot64(b15, b16));
    (*s)[20] = xor64(b20, andnot64(b21, b22)); (*s)[21] = xor64(b21, andnot64(b22, b23));
    (*s)[22] = xor64(b22, andnot64(b23, b24)); (*s)[23] = xor64(b23, andnot64(b24, b20));
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  for (var k: u32 = 0u; k < ITERATIONS; k = k + 1u) {
    ...
    // Build absorbed state (1 block, 136 bytes)
    var st: array<vec2<u32>, 25>;

    // lanes 0..3 ← challenge (32 bytes)
    ...
    // lanes 4..6 ← zero (high 24 bytes of uint256 nonce)
    st[4] = vec2<u32>(0u, 0u);
    st[5] = vec2<u32>(0u, 0u);
    st[6] = vec2<u32>(0u, 0u);

    // lane 7 ← nonce as BE 8 bytes
    st[7] = vec2<u32>(bswap32(n_hi), bswap32(n_lo));

    // lane 8 ← 0x01 (keccak pad delimiter)
    st[8] = vec2<u32>(0x00000001u, 0x00000000u);

    // lanes 9..15 ← zero
    for (var i: u32 = 9u; i <= 15u; i++) { st[i] = vec2<u32>(0u, 0u); }

    // lane 16 ← 0x80 (final pad byte)
    st[16] = vec2<u32>(0u, 0x80000000u);

    // lanes 17..24 ← zero (capacity)
    for (var i: u32 = 17u; i <= 24u; i++) { st[i] = vec2<u32>(0u, 0u); }

    keccak_f1600(&st);

    // Compare hash < difficulty (uint256 MSB-first)
    let h0 = bswap32(st[0].x); let h1 = bswap32(st[0].y);
    let h2 = bswap32(st[1].x); let h3 = bswap32(st[1].y);
    let h4 = bswap32(st[2].x); let h5 = bswap32(st[2].y);
    let h6 = bswap32(st[3].x); let h7 = bswap32(st[3].y);

    let d0 = u.difficulty[0].x; let d1 = u.difficulty[0].y;
    let d2 = u.difficulty[0].z; let d3 = u.difficulty[0].w;
    let d4 = u.difficulty[1].x; let d5 = u.difficulty[1].y;
    let d6 = u.difficulty[1].z; let d7 = u.difficulty[1].w;

    var lt = false; var settled = false;
